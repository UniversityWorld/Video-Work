...
class ShaderWrapper(object):
    def __init__(
        ...
        gradient_size = False,     # changed
        gradient_points = 2,     # changed
    ):
        self.ctx = ctx
        ...
        self.gradient_size = gradient_size      # changed
        self.gradient_points = gradient_points      # changed
    ...
    def init_program_code(self) -> None:     # changed
        def get_code(name: str) -> str | None:
            shader_code = get_shader_code_from_file(
                os.path.join(self.shader_folder, f"{name}.glsl")
            )
            if self.gradient_size:
                shader_code = shader_code.replace("~gradient_size", str(self.gradient_size))
                shader_code = shader_code.replace("~gradient_points", str(self.gradient_points))
            return shader_code
